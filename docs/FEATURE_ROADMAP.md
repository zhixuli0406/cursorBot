...
| ⬜ | WhatsApp | Baileys | - | 🔴 高 | 最廣泛使用 |
| ⬜ | iMessage | imsg CLI | - | 🟡 中 | macOS only |
| ⬜ | Slack | Plugin | - | 🟡 中 | 企業用戶 |
...
| ⬜ | AWS Bedrock | Claude on AWS | 🟡 中 | 企業需求 |
| ⬜ | Moonshot | 月之暗面 | 🟢 低 | 中國市場 |
| ⬜ | Minimax | - | 🟢 低 | 中國市場 |
| ⬜ | GLM (智譜) | ChatGLM | 🟢 低 | 中國市場 |

### 待辦事項

- [ ] 實現 AWS Bedrock Provider
- [ ] 評估中國 AI 提供者需求
...
| ✅ | 廣播訊息 | 多用戶通知 | - | v0.2 新增 |
| 🟡 | 群組權限 | 權限控制 | 🟡 中 | 需加強 |
| ⬜ | Channel Routing | 頻道路由 | 🟡 中 | 多頻道支援 |
| ⬜ | Presence | 在線狀態 | 🟢 低 | 顯示狀態 |
...
| ⬜ | Chrome Extension | 瀏覽器擴展 | 🟡 中 | 深度整合 |
| ✅ | Sandbox | 沙盒執行 | - | v0.3 新增 |
| ✅ | Subagents | 子代理 | - | v0.3 新增 |
| 🟡 | LLM Task | LLM 任務工具 | 🟡 中 | 需加強 |
| ⬜ | Apply Patch | 應用補丁 | 🟡 中 | Git 整合 |
| ⬜ | Agent Send | 代理發送 | 🟢 低 | 代理通訊 |
| ⬜ | Reactions | 表情回應 | 🟢 低 | UX 增強 |
...
| ✅ | Markdown | 格式化 | - | 已完成 |
| 🟡 | Chunking | 分塊傳輸 | 🟡 中 | 需加強 |
| ⬜ | Draft Streaming | 草稿串流 | 🟢 低 | Telegram 特有 |
...
| ⬜ | Pairing | 設備配對 | 🟢 低 | 多設備 |
| ⬜ | WebSocket | 即時通訊 | 🟡 中 | 雙向通訊 |
| ⬜ | Multiple Gateways | 多閘道 | 🟢 低 | 高可用 |
| ⬜ | Discovery | 服務發現 | 🟢 低 | 自動發現 |
| ⬜ | Bonjour | mDNS | 🟢 低 | 區網發現 |
| ⬜ | Tailscale | VPN 整合 | 🟡 中 | 遠端存取 |
| ⬜ | Remote Gateway | 遠端閘道 | 🟡 中 | 遠端部署 |
...
| 狀態 | 功能 | 說明 | 優先級 | 備註 |
|:----:|------|------|:------:|------|
| ⬜ | Web Dashboard | 網頁儀表板 | 🔴 高 | 管理介面 |
| ⬜ | Control UI | 控制介面 | 🔴 高 | 設定管理 |
| ⬜ | WebChat | 網頁聊天 | 🟡 中 | 網頁版 |
| ⬜ | TUI | 終端介面 | 🟢 低 | CLI UI |
| ⬜ | Canvas | 畫布功能 | 🟢 低 | 視覺化 |
...
|:----:|------|------|:------:|------|
| ✅ | Status | 狀態查詢 | - | /status |
| ✅ | Logs | 日誌查看 | - | docker logs |
| 🟡 | Setup | 安裝設定 | 🟡 中 | 腳本形式 |
| 🟡 | Configure | 配置管理 | 🟡 中 | .env 形式 |
| 🟡 | Doctor | 診斷 | 🟡 中 | /diagnose |
| 🟡 | Message | 發送訊息 | 🟡 中 | API 形式 |
| 🟡 | Sessions | 會話管理 | 🟡 中 | 部分實現 |
| ⬜ | Onboard | 引導設定 | 🟡 中 | 互動式 |
| ⬜ | Dashboard | 儀表板 | 🟡 中 | CLI 版 |
| ⬜ | Reset | 重置 | 🟢 低 | 清除資料 |
